...
describe("stringify()", function() {
  ...
  it("stringifies a nested object", function(done) {
    ...
    expect(Qs.stringify({ a: { b: { c: { d: "e" } } } })).to.equal(
  ...
  it("stringifies an array value", function(done) {
    expect(Qs.stringify({ a: ["b", "c", "d"] })).to.equal(
  ...
  it("omits nulls when asked", function(done) {
    expect(Qs.stringify({ a: "b", c: null }, { skipNulls: true })).to.equal(
      "a=b"
    );
    done();
  });
  ...
  it("omits array indices when asked", function(done) {
    expect(Qs.stringify({ a: ["b", "c", "d"] }, { indices: false })).to.equal(
      "a=b&a=c&a=d"
    );
    done();
  });
  ...
  it("stringifies an object inside an array", function(done) {
    ...
    expect(Qs.stringify({ a: [{ b: { c: [1] } }] })).to.equal(
  ...
  it("does not omit object keys when indices = false", function(done) {
    expect(Qs.stringify({ a: [{ b: "c" }] }, { indices: false })).to.equal(
      "a%5Bb%5D=c"
    );
    done();
  });
  ...
  it("uses indices notation for arrays when indices=true", function(done) {
    expect(Qs.stringify({ a: ["b", "c"] }, { indices: true })).to.equal(
  ...
  it("uses repeat notation for arrays when no arrayFormat=repeat", function(done) {
    expect(Qs.stringify({ a: ["b", "c"] }, { arrayFormat: "repeat" })).to.equal(
      "a=b&a=c"
    );
    done();
  });
  ...
  it("stringifies an empty value", function(done) {
    ...
    expect(Qs.stringify({ a: null }, { strictNullHandling: true })).to.equal(
      "a"
    );
  ...
  it("stringifies the weird object from qs", function(done) {
    expect(Qs.stringify({ "my weird field": "~q1!2\"'w$5&7/z8)?" })).to.equal(
  ...
  it("stringifies buffer values", function(done) {
    ...
    expect(Qs.stringify({ a: { b: new Buffer("test") } })).to.equal(
      "a%5Bb%5D=test"
    );
    done();
  });
  ...
  it("stringifies an object using an alternative delimiter", function(done) {
    expect(Qs.stringify({ a: "b", c: "d" }, { delimiter: ";" })).to.equal(
      "a=b;c=d"
    );
    done();
  });
  ...
  it("supports custom representations when filter=function", function(done) {
    ...
    expect(Qs.stringify(obj, { filter: filterFunc })).to.equal(
  ...
  it("can disable uri encoding", function(done) {
    ...
    expect(Qs.stringify({ a: { b: "c" } }, { encode: false })).to.equal(
      "a[b]=c"
    );
    expect(
      Qs.stringify(
  ...
  it("can sort the keys", function(done) {
    ...
    expect(Qs.stringify({ a: "c", z: "y", b: "f" }, { sort: sort })).to.equal(
      "a=c&b=f&z=y"
    );
    expect(
