...
describe("parse()", function() {
  ...
  it("allows enabling dot notation", function(done) {
    ...
    expect(Qs.parse("a.b=c", { allowDots: true })).to.deep.equal({
      a: { b: "c" }
    });
    done();
  });
  ...
  it("only parses one level when depth = 1", function(done) {
    expect(Qs.parse("a[b][c]=d", { depth: 1 })).to.deep.equal({
      a: { b: { "[c]": "d" } }
    });
    expect(Qs.parse("a[b][c][d]=e", { depth: 1 })).to.deep.equal({
      a: { b: { "[c][d]": "e" } }
    });
    done();
  });
  ...
  it("transforms arrays to objects", function(done) {
    ...
    ).to.deep.equal({ foo: [{ a: "a", b: "b" }, { a: "aa", b: "bb" }] });
  ...
  it("transforms arrays to objects (dot notation)", function(done) {
    ...
    ).to.deep.equal({ foo: [{ baz: "bar" }], fool: { bad: "baz" } });
    expect(
      Qs.parse("foo[0].baz=bar&fool.bad.boo=baz", { allowDots: true })
    ).to.deep.equal({ foo: [{ baz: "bar" }], fool: { bad: { boo: "baz" } } });
    expect(
      Qs.parse("foo[0][0].baz=bar&fool.bad=baz", { allowDots: true })
    ).to.deep.equal({ foo: [[{ baz: "bar" }]], fool: { bad: "baz" } });
    expect(
      Qs.parse("foo[0].baz[0]=15&foo[0].bar=2", { allowDots: true })
    ).to.deep.equal({ foo: [{ baz: ["15"], bar: "2" }] });
    expect(
      Qs.parse("foo[0].baz[0]=15&foo[0].baz[1]=16&foo[0].bar=2", {
        allowDots: true
      })
    ).to.deep.equal({ foo: [{ baz: ["15", "16"], bar: "2" }] });
    expect(
      Qs.parse("foo.bad=baz&foo[0]=bar", { allowDots: true })
    ).to.deep.equal({ foo: { bad: "baz", "0": "bar" } });
    expect(
      Qs.parse("foo.bad=baz&foo[]=bar", { allowDots: true })
    ).to.deep.equal({ foo: { bad: "baz", "0": "bar" } });
    expect(
      Qs.parse("foo[]=bar&foo.bad=baz", { allowDots: true })
    ).to.deep.equal({ foo: { "0": "bar", bad: "baz" } });
    expect(
      Qs.parse("foo.bad=baz&foo[]=bar&foo[]=foo", { allowDots: true })
    ).to.deep.equal({ foo: { bad: "baz", "0": "bar", "1": "foo" } });
    expect(
      Qs.parse("foo[0].a=a&foo[0].b=b&foo[1].a=aa&foo[1].b=bb", {
        allowDots: true
      })
  ...
  it("parses semi-parsed strings", function(done) {
    expect(Qs.parse({ "a[b]": "c" })).to.deep.equal({ a: { b: "c" } });
    expect(Qs.parse({ "a[b]": "c", "a[d]": "e" })).to.deep.equal({
      a: { b: "c", d: "e" }
    });
    done();
  });
  ...
  it("allows overriding parameter limit", function(done) {
    expect(Qs.parse("a=b&c=d", { parameterLimit: 1 })).to.deep.equal({
      a: "b"
    });
    done();
  });
  ...
  it("allows overriding array limit", function(done) {
    expect(Qs.parse("a[0]=b", { arrayLimit: -1 })).to.deep.equal({
      a: { "0": "b" }
    });
    expect(Qs.parse("a[-1]=b", { arrayLimit: -1 })).to.deep.equal({
      a: { "-1": "b" }
    });
  ...
  it("can allow overwriting prototype properties", function(done) {
    ...
    ).to.deep.equal({ a: { hasOwnProperty: "b" } }, { prototype: false });
    expect(
      Qs.parse("hasOwnProperty=b", { allowPrototypes: true })
    ).to.deep.equal({ hasOwnProperty: "b" }, { prototype: false });
    done();
  });
  ...
  it("can return plain objects", function(done) {
    ...
    expect(Qs.parse(null, { plainObjects: true })).to.deep.equal(
      Object.create(null)
    );
    ...
    expectedArray.a["0"] = "b";
    expectedArray.a.c = "d";
    expect(Qs.parse("a[]=b&a[c]=d", { plainObjects: true })).to.deep.equal(
      expectedArray
    );
    done();
  });
